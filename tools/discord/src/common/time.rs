//! Canonical UTC timestamp generation without a date/time crate. The store
//! re-normalizes every timestamp through SQLite `datetime()` at its write
//! boundary, so this only needs to emit a form SQLite can parse — a
//! fixed-width `YYYY-MM-DDTHH:MM:SSZ` UTC string satisfies that and matches the
//! rest of the crate's RFC3339 convention.
...
/// a date crate for the one conversion the daemon needs.
